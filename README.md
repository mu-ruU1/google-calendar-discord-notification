# google-calendar-discord-notification

Googleカレンダー Discord通知Bot

![bot](./docs/img/image.jpg)

## 準備

1. [Google Calendar API](./docs/google.md)
2. [Discord Bot](./docs/discord.md)
3. [GitHub Actions](./docs/github.md)

## Docs

### google-api-go-client

- [Google API Go Client](https://pkg.go.dev/google.golang.org/api)
- [Google API Go Client - calendar/v3](https://pkg.go.dev/google.golang.org/api/calendar/v3)
- [Google API Go Client - option](https://pkg.go.dev/google.golang.org/api/option)

### DiscordGo

- [DiscordGo](https://pkg.go.dev/github.com/bwmarrin/discordgo)
- [DiscordGo - GitHub Wiki FAQ](https://github.com/bwmarrin/discordgo/wiki/FAQ)
